...
import { analyzeImageForTomato } from '@/utils/imageAnalysis';
...
const Index = () => {
  ...
  const handleImageUpload = async (file: File) => {
    ...
    try {
      // First, analyze the image to check if it's a tomato
      console.log('Starting image analysis...');
      const analysisResult = await analyzeImageForTomato(previewUrl);
      console.log('Analysis result:', analysisResult);

      if (!analysisResult.isTomato) {
        // Not a tomato image - show error and return to upload state
        console.log('Image rejected - not a tomato');
        toast({
          title: "Tomato Plant Required",
          description: "Please upload a clear photo of a tomato plant (with red/yellow fruits and green leaves) to identify diseases. Images of other objects or plants are not supported.",
          variant: "destructive",
        });
        setCurrentState('upload');
        return;
              ...
              <Card className="p-10 bg-card/70 backdrop-blur-md border-primary/30 shadow-lifted">
                <div className="text-center mb-8">
                  ...
                  <p className="text-muted-foreground text-lg">
                    Take a clear photo of the affected tomato plant parts for accurate disease diagnosis
                  </p>
                </div>
